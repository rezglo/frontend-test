import reducer, { channelsListAction } from "../reducers/channelsListReducer";
...
test("should handle to empty list", () => {
  ...
  expect(
    reducer(previousState, channelsListAction({ payload: "Run the tests" }))
...
test("should handle a new value to user list", () => {
  ...
  expect(
    reducer(previousState, channelsListAction({ payload: "Use Redux" }))
