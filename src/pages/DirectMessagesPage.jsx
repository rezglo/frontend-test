...
function DirectMessagesPage() {
  ...
  return (
    <div id="content" className="h-full w-full flex flex-row">
      <div
        id="sidebar"
        className={`${showOnMobile ? 'max-[981px]:hidden' : 'max-[981px]:visible'} w-[40%] max-[981px]:w-full min-[981px]:max-w-[325px] h-full max-[981px]:bg-white bg-violet-100 rounded-l-md max-[981px]:rounded-r-md pt-4`}
      >
      ...
      <div
        id="chat"
        className={`${showOnMobile ? 'max-[981px]:visible' : 'max-[981px]:hidden'} w-full h-full bg-white min-[981px]:rounded-r-md max-[981px]:rounded-t-lg`}
      >
        <ChatComponent />
      </div>
    </div>
