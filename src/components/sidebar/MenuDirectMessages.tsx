...
import {
  TYPE_DIRECT_MESSAGE_ACTION,
  useDirectMessage,
} from '@/features/dashboard/directMessage/hooks/useDirectMessage'
import { StatusDirectMessage } from '@/features/dashboard/directMessage/types/index.d'

export const MenuDirectMessages = () => {
  const { isOpen: directMessagesExpanded, toggle: toogleDirectMessagesExpanded } =
    useDisclosure(true)
  ...
  return (
    <>
      <ListItemButton onClick={toogleDirectMessagesExpanded} className="text-white opacity-60">
                    ...
                    <ListItemIcon className="text-xl justify-center mr-2 min-w-[24px] text-white relative">
                      <FiberManualRecord
                        className={`absolute bottom-[-8px] right-[-8px] z-50 ${
                          status === StatusDirectMessage.ONLINE ? 'text-green-500' : 'text-gray-500'
                        }`}
                      />
